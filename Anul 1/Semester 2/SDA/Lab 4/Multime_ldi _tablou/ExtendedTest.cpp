#include "Multime.h"
#include <assert.h>
#include "ExtendedTest.h"
#include "MultimeIterator.h"
#include <iostream>

using namespace std;

void printM(Multime& s){
	MultimeIterator it = s.iterator();
	while (it.valid()){
		std::cout << it.getCurrent() << " ";
		it.next();
	}
	std::cout << std::endl;
	std::cout << std::endl;
}

void testCreate() {
	Multime s;
	assert(s.size() == 0);
	assert(s.isEmpty() == true);

	for (int i = -10; i < 10; i++) { //search in an empty Multime
	    //std::cout<<s.search(i);
		assert(s.search(i) == false);
	}
	for (int i = -10; i < 10; i++) { //delete from an empty Multime
		assert(s.remove(i) == false);
	}

	MultimeIterator it = s.iterator(); //iterator on the empty Multime should be invalid from the start
	assert(it.valid() == false);
}


void testAdd() {
	Multime s; //adaugam elemente [0, 10)
	for (int i = 0; i < 10; i++) {
		s.add(i);
	}
	//printM(m);
	assert(s.isEmpty() == false);
	assert(s.size() == 10);
	for (int i = -10; i < 20; i++) { //add more elements [-10, 20)
		s.add(i);
	}
	//printM(m);
	assert(s.isEmpty() == false);
	assert(s.size() == 30);
	for (int i = -100; i < 100; i++) { //add more elements [-100, 100)
		s.add(i);
	}
	//printM(m);
	assert(s.isEmpty() == false);
	assert(s.size() == 200);
	for (int i = -200; i < 200; i++) {
		if (i < -100) {
			assert(s.search(i) == false);
		}
		else if (i < 0) {
			assert(s.search(i) == true);
		}
		else if (i < 100) {
			assert(s.search(i) == true);
		}
		else {
			assert(s.search(i) == false);
		}
	}
	for (int i = 10000; i > -10000; i--) { //add larger values first
		s.add(i);
	}
	assert(s.size()==20000);
}


void testRemove() {
	Multime m;
	for (int i = -100; i < 100; i++) { //delete from the emtpy Multime
		assert(m.remove(i) == false);
	}
	assert(m.size() == 0);

	//printM(m);

	for (int i = -100; i < 100; i = i + 2) {
		m.add(i);
	}

	for (int i = -100; i < 100; i=i+2) { //delete everything (including inexistent elements)
		if (i % 2 == 0) {
		    //std::cout<<m.remove(i);
			//assert(m.remove(i) == true);
		}
		else {
			assert(m.remove(i) == false);
		}
	}
	assert(m.size() == 99);
	//printM(m);

	for (int i = -100; i <= 100;i++) {
		m.add(i);
	}
	//printM(m);
	for (int i = 100; i > -100; i--) { //delete in descending order (reverse order compared to the adding)
        assert(m.remove(i) == true);

    }

	//printM(m);

	m.remove(-100);
	//assert(m.size() == 0);

	for (int i = -100; i < 100; i++) { //add each element five times
		m.add(i);
		m.add(i);
		m.add(i);
		m.add(i);
		m.add(i);
	}
	//printM(m);

	for (int i = -1; i < 1; i++) { //delete existing and inexistent elements
			assert(m.remove(i) == true);
	}
    assert(m.size() != 0);

}


void testIterator() {
	Multime s;
	MultimeIterator it = s.iterator(); //iterator on an empty Multime
	assert(it.valid() == false);

	for (int i = 0; i < 100; i++) {  
		s.add(33);
	}
	//printM(m);
	MultimeIterator it2 = s.iterator(); 
	assert(it2.valid() == true);
	TElem elem = it2.getCurrent();
	assert(elem == 33);
	it2.next();
	assert(it2.valid() == false);

	it2.first(); 
	assert(it2.valid() == true);

	Multime s2;
	for (int i = -100; i < 100; i++) { 
		s2.add(i);
		s2.add(i);
		s2.add(i);
	}
	MultimeIterator it3 = s2.iterator();
	assert(it3.valid() == true);
	for (int i = 0; i < 200; i++) {
		it3.next();
	}
	it3.first();
	assert(it3.valid() == true);


	Multime s3;
	for (int i = 0; i < 200; i= i + 4) { 
		s3.add(i);
	}

	MultimeIterator it4 = s3.iterator();
	assert(it4.valid() == true);
	int count = 0;
	while (it4.valid()) { 
		TElem e = it4.getCurrent();
		assert(e % 4 == 0);
		it4.next();
		count++;
	}
	assert(count == 49);

}



void testQuantity() {//add lots of elements
	Multime s;
	for (int i = 10; i >= 1; i--) {
		for (int j = -30000; j < 30000; j = j + i) {
			s.add(j);
		}
	}
	assert(s.size() == 60000);
	MultimeIterator it = s.iterator();
	assert(it.valid() == true);
	for (int i = 0; i < s.size(); i++) {
		it.next();
	}
	it.first();
	while (it.valid()) { 
		TElem e = it.getCurrent();
		assert(s.search(e) == true);
		it.next();
	}
	assert(it.valid() == false);

    for (int i = 0; i < 10; i++) { //delete existing and inexistent elements
        for (int j = 40000; j >= -40000; j--) {
            s.remove(j);
        }
    }
}


void testAllExtended() {
	testCreate();
	testAdd();
	testRemove();
	testIterator();
	testQuantity();

}